...
export function StaffManager({ agencyId }: StaffManagerProps) {
  ...
  const openEditStaffDialog = (staff: StaffMember) => {
  ...
  const handleUpdateStaff = async () => {
    ...
    try {
      const res = await fetch(`/api/agency/staff/${editingStaffId}`, {
        ...
        body: JSON.stringify({ fullName: editFullName.trim(), isActive: editIsActive, role: editRole }),
      });
      if (res.ok) {
  ...
  const handleCreateStaff = async () => {
    if (!newStaffUsername.trim() || !newStaffFullName.trim() || !newStaffPassword || !agencyId) return;
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setCreatedCredentials({ username: data.staff.user.username, password: data.initialPassword });
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={addStaffOpen} onOpenChange={(open) => { setAddStaffOpen(open); if (!open) resetStaffForm(); }}>
          <DialogTrigger asChild>
            <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg h-8 text-xs">
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4 py-2">
              <div className="space-y-2">
                ...
                <Input value={newStaffUsername} onChange={(e) => setNewStaffUsername(e.target.value)} placeholder={t('enterUsername')} className="h-11" dir="ltr" />
              </div>
              <div className="space-y-2">
                <Label>{t('staffFullName')}</Label>
                <Input value={newStaffFullName} onChange={(e) => setNewStaffFullName(e.target.value)} placeholder={t('staffFullName')} className="h-11" />
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                ...
                <Input type="text" value={newStaffPassword} onChange={(e) => setNewStaffPassword(e.target.value)} placeholder={t('passwordMinLength')} className="h-11" dir="ltr" />
              </div>
              <div className="space-y-2">
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => { setAddStaffOpen(false); resetStaffForm(); }}>{t('cancel')}</Button>
              <Button className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={handleCreateStaff} disabled={addStaffLoading || !newStaffUsername.trim() || !newStaffFullName.trim() || !newStaffPassword}>
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`h-9 w-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
                    isOwner ? 'bg-amber-100 dark:bg-amber-900/30' : staff.role === 'MANAGER' ? 'bg-purple-100 dark:bg-purple-900/30' : 'bg-emerald-100 dark:bg-emerald-900/30'
                  }`}>
                    <Shield className={`h-4.5 w-4.5 ${isOwner ? 'text-amber-700 dark:text-amber-400' : staff.role === 'MANAGER' ? 'text-purple-700 dark:text-purple-400' : 'text-emerald-700 dark:text-emerald-400'}`} />
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-medium text-foreground truncate">{staff.user.fullName}</p>
                      {isOwner && <span className="text-[9px] font-bold bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 px-1.5 py-0.5 rounded-full flex-shrink-0">{t('ownerRole')}</span>}
                      {!isOwner && staff.role === 'MANAGER' && <span className="text-[9px] font-bold bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-400 px-1.5 py-0.5 rounded-full flex-shrink-0">{t('staffRoleManager')}</span>}
                      {!isOwner && staff.role === 'STAFF' && <span className="text-[9px] font-bold bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 px-1.5 py-0.5 rounded-full flex-shrink-0">{t('staffRoleStaff')}</span>}
                      {!isUserActive && <span className="text-[9px] font-bold bg-gray-200 dark:bg-gray-700/60 text-gray-500 dark:text-gray-400 px-1.5 py-0.5 rounded-full flex-shrink-0">{t('inactive')}</span>}
                    </div>
                    <p className="text-xs text-muted-foreground">@{staff.user.username}</p>
                    {staff.user.role === 'AGENCY_STAFF' && !isOwner && <p className="text-[10px] text-emerald-600 dark:text-emerald-400 mt-0.5">{t('initialAccountCreated')}</p>}
                  </div>
                </div>
                  ...
                  {!isOwner && (
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 flex-shrink-0 rounded-full transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:ring-offset-2 dark:focus:ring-offset-gray-900 cursor-pointer ${
                        isUserActive ? 'bg-emerald-500 shadow-inner shadow-emerald-600/30' : 'bg-gray-300 dark:bg-gray-600 shadow-inner shadow-gray-400/30'
                      }`}
                    >
                      <span className={`pointer-events-none absolute top-[2px] h-5 w-5 rounded-full shadow-md bg-white transition-all duration-300 ease-in-out ${isUserActive ? 'start-[22px]' : 'start-[2px]'}`} />
                    </button>
                  )}
                  {!isOwner && (
                    <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-muted-foreground hover:text-foreground" onClick={() => openEditStaffDialog(staff)}>
                      <Edit3 className="h-3.5 w-3.5" />
                    </Button>
                  )}
                  {!isOwner && (
                    <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-red-500 hover:text-red-600" onClick={() => handleRemoveStaff(staff.id)}>
          ...
          {createdCredentials && (
            <div className="space-y-4 py-2">
              ...
              <div className="flex gap-2">
                ...
                >
                  {credentialsCopied ? (
                    <><CheckCircle2 className="h-4 w-4 me-1" />{t('copied')}</>
                  ) : (
                    <><Copy className="h-4 w-4 me-1" />{t('copyCredentials')}</>
                  )}
                </Button>
              </div>
              <p className="text-xs text-amber-600 dark:text-amber-400 text-center">{t('credentialsWarning')}</p>
            </div>
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={editStaffOpen} onOpenChange={(open) => { setEditStaffOpen(open); if (!open) setEditingStaffId(null); }}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label>{t('staffFullName')}</Label>
              <Input value={editFullName} onChange={(e) => setEditFullName(e.target.value)} placeholder={t('staffFullName')} className="h-11" />
            </div>
            <div className="space-y-2">
              <Label>{t('staffRoleSelect')}</Label>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                ...
                className={`relative inline-flex h-7 w-12 flex-shrink-0 rounded-full transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:ring-offset-2 dark:focus:ring-offset-gray-900 cursor-pointer ${
                  editIsActive ? 'bg-emerald-500 shadow-inner shadow-emerald-600/30' : 'bg-gray-300 dark:bg-gray-600 shadow-inner shadow-gray-400/30'
                }`}
              >
                <span className={`pointer-events-none absolute top-[3px] h-5 w-5 rounded-full shadow-md bg-white transition-all duration-300 ease-in-out ${editIsActive ? 'start-[25px]' : 'start-[3px]'}`} />
              </button>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setEditStaffOpen(false); setEditingStaffId(null); }}>{t('cancel')}</Button>
            <Button className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={handleUpdateStaff} disabled={editStaffLoading || !editFullName.trim()}>
